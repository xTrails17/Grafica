function [ out ] = ruota_nearest_semplice( img , gradi)
    %ruota ruota un immagine applicando un interpolazione di tipo nearest
    %neighbour ma scorrendo l'intera bounding box dell'immagine di output.
    [x, y]=size(img);

    gradi=(gradi/180)*pi;
    kernel=[cos(-gradi) sin(-gradi) ; 
           -sin(-gradi) cos(-gradi)];
    c1=[x y]./2;
